...
Para acessar os dados gerais da sua integração, navegue até o [Painel do desenvolvedor](/developers/panel/app) e clique sobre o card de uma aplicação para acessar os seus dados e encontrar todos os recursos e informações úteis. Veja abaixo quais informações serão apresentadas em tela.

![Dados da integração](/images/cow/integration-data-pro-pt-v1.png)

:::AccordionComponent{title="Credenciais"}

Esta seção exibe as :toolTipComponent[*credenciais*]{link="/developers/pt/docs/checkout-pro/additional-content/credentials" linkText="Credenciais" content="Chaves de acesso únicas que usamos para identificar uma integração na sua conta, estando vinculadas à sua aplicação. Para mais informações, acesse o link abaixo."} da sua aplicação, sendo elas:

- **Credenciais de teste**: um conjunto de chaves que são utilizadas tanto na etapa de desenvolvimento, para garantir configurações seguras, quanto na etapa de testes, para testar a integração. Ao criar uma aplicação as credenciais de teste serão geradas automaticamente.
- **Credenciais de produção**: um conjunto de chaves que permitem receber pagamentos reais em lojas e em outras aplicações. Para obter as credenciais de produção, você deverá **ativá-las** clicando em **Ativar credenciais** e preenchendo alguns dados sobre o seu negócio.

Para mais informações, acesse a documentação de [Credenciais](/developers/pt/docs/checkout-pro/additional-content/credentials).

:::
:::AccordionComponent{title="Detalhes da aplicação"}
...
Esta seção exibe os dados básicos da aplicação, incluindo:
  ...
  - **Produto integrado**: o produto ou plataforma integrada com a aplicação. 
...
No menu à esquerda da tela, você pode **clicar no botão de edição que está acima do nome de sua aplicação** e editar as **configurações básicas e avançadas** que incluem os dados da sua aplicação e o produto a ser integrado. São elas:
...
* **Solução de pagamento a ser integrada**: edite a solução de pagamento a ser integrada entre **Pagamentos online** e **Pagamentos presenciais**.
  - **Pagamentos online**: se você pretende utilizar uma plataforma de comércio eletrônico, marque **Sim** e, em seguida, selecione a **plataforma** com a qual você irá integrar. Por fim, escolha o **produto** que está integrando. Caso você não esteja utilizando uma plataforma de comércio eletrônico, marque **Não** e selecione o **produto** que está integrando. Opcionalmente, você poderá selecionar o(s) modelo(s) de integração.
  - **Pagamentos presenciais**: selecione o **produto** que você está integrando. Se você selecionar a opção QR Code, opcionalmente você também poderá escolher o(s) modelo(s) de integração.
...
:::
:::AccordionComponent{title="Qualidade da integração"}
 ...
 * **Automática:** do dia 1 ao 7 de cada mês, o Mercado Pago realiza uma medição periódica de qualidade para todas as integrações com **Checkout Pro** que tenham ao menos um pagamento realizado com credenciais de produção.

> WARNING
>
> Integrações com **Plugins e Plataformas** não poderão ser avaliadas.
...
:::
:::AccordionComponent{title="Notificações"}

Caso você já tenha [configurado as suas notificações Webhooks](/developers/pt/docs/checkout-pro/payment-notifications), nessa seção será exibido o percentual de notificações entregues com sucesso para a integração em questão.
